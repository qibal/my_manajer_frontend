...
export default function AssetManagementPage() {
  ...
  // Load assets data using import
  useEffect(() => {
    try {
      setAssets(assetData);
      toast.success('Data asset berhasil dimuat!');
    } catch (error) {
        ...
        <div className="space-y-4">
          ...
          <div className="flex space-x-1">
            {Array.from({ length: 20 }).map((_, i) => (
              <Skeleton key={i} className="h-8 w-1 rounded" />
            ))}
          </div>
        </div>
      </CardContent>
      ...
      <CardContent className="p-4">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500">{asset.duration} • {asset.format}</p>
          </div>
          <Button
            size="sm"
            variant="ghost"
            onClick={() => playAudio(asset.id)}
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600">{asset.artist}</p>
          </div>
          <Button
            size="sm"
            variant="ghost"
        ...
        <div className="flex items-end space-x-1 h-12 mb-4">
          {asset.waveform_data.map((height, index) => (
            <div
              key={index}
        ...
        <div className="flex items-center justify-between text-xs text-gray-500">
          ...
          <span>{asset.genre} • {asset.mood}</span>
        </div>
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Kelola semua aset untuk semua bisnis Anda</p>
        </div>
        <div className="flex items-center space-x-3">
      ...
      <div className="relative max-w-2xl mx-auto">
        ...
        <Input
          placeholder="Cari asset berdasarkan nama, deskripsi, atau tag..."
          value={searchQuery}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="sound_effects" className="flex items-center space-x-2">
        ...
        <TabsContent value="sound_effects" className="mt-6">
          {loading ? (
            <div className={`grid gap-4 ${viewMode === 'card' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1'}`}>
              {Array.from({ length: 8 }).map((_, i) => (
                <SoundEffectSkeleton key={i} />
              ))}
            </div>
          ) : (
            <div className={`grid gap-4 ${viewMode === 'card' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1'}`}>
              {getFilteredAssets('sound_effects').map((asset) => (
                <SoundEffectCard key={asset.id} asset={asset} />
              ))}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="music" className="mt-6">
          {loading ? (
            ...
            <div className={`grid gap-6 ${viewMode === 'card' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              {getFilteredAssets('music').map((asset) => (
                <MusicCard key={asset.id} asset={asset} />
              ))}
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="photos" className="mt-6">
          {loading ? (
            ...
            <div className={`grid gap-4 ${viewMode === 'card' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'grid-cols-1'}`}>
              {getFilteredAssets('photos').map((asset) => (
